...
TMP_N0=$(mktemp /tmp/dw-node-multi-fwd-1-XXX.dat)
TMP_N1=$(mktemp /tmp/dw-node-multi-fwd-2-XXX.dat)
TMP_N2=$(mktemp /tmp/dw-node-multi-fwd-3-XXX.dat)
...
# branched multi-fwd
client -F :7892,branch -C 1111 -F:7893,branch -C 2222 -R | grep -q "Sent pkts - success: 1, failed: 0, thr_id: 0"
cat $TMP_N1 | grep -q "COMPUTE(1111us)->REPLY(512b)->EOM"
cat $TMP_N2 | grep -q "COMPUTE(2222us)->REPLY(512b)->EOM"

# combined
TMP_N3=$(mktemp /tmp/dw-node-multi-fwd-4-XXX.dat)
node_bg -b :7894 &> $TMP_N3

client -F :7892,branch -C 2222 -F :7893,:7894,branch -C 3333 -R | grep -q "Sent pkts - success: 1, failed: 0, thr_id: 0"
...
tmp_client=$(mktemp /tmp/dw-client-multi-fwd-XXX.dat)
