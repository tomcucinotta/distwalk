...
void* storage_worker(void* args) {
    ...
    check(dw_poll_init(p_poll, DW_EPOLL) == 0);
    ...
    if (dw_poll_add(p_poll, infos->terminationfd, DW_POLL_IN, i2l(TERMINATION, infos->terminationfd)) < 0)
...
void* conn_worker(void* args) {
    ...
    // Add termination fd
    check(dw_poll_add(&infos->dw_poll, infos->terminationfd, DW_POLL_IN, i2l(TERMINATION, infos->terminationfd)) == 0);

    // Add timer fd
    check(dw_poll_add(&infos->dw_poll, infos->timerfd, DW_POLL_IN, i2l(TIMER, infos->timerfd)));

    // Add stat fd
    if (infos->statfd != -1)
        check(dw_poll_add(&infos->dw_poll, infos->statfd, DW_POLL_IN, i2l(STATS, infos->statfd)));

    // Add storage reply fd
    if (infos->storefd != -1) {
        if (dw_poll_add(&infos->dw_poll, infos->store_replyfd, DW_POLL_IN, i2l(STORAGE, infos->store_replyfd)) < 0)
            perror("dw_poll_add(): storefd failed");
    }

    while (running) {
        dw_log("epoll_wait()ing...\n");
        int rv = dw_poll_wait(&infos->dw_poll);
        if (rv != 0) {
            perror("dw_poll_wait()");
        ...
        uint64_t aux;
        dw_poll_flags pflags;
        while (dw_poll_next(&infos->dw_poll, &pflags, &aux) != 0) {
            int fd;
            event_t type;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < input_args.num_threads; i++) {
        ...
        * case) */
        ...
        check(dw_poll_init(&conn_worker_infos[i].dw_poll, DW_EPOLL) == 0);
