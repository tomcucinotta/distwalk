...
rm /tmp/dw_client_computes_*.log

../src/dw_node --nt $NUM_THREADS &> /tmp/dw_node_computes.log &
NODEPID=$!
...
do
	../src/dw_client -C 1000 -n 50 &> /tmp/dw_client_computes_$i.log &
    CLIENTPIDS[${i}]=$!
done
...
echo "Full results in /tmp/dw_client_computes_*.log"
tail -n -10 /tmp/dw_client_computes_*.log
