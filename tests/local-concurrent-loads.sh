...
rm /tmp/dw_client_loads_*.log

../src/dw_node --nt $NUM_THREADS -s ../src/test.data &> /tmp/dw_node_loads.log &
NODEPID=$!

sleep 1
../src/dw_client -S 10000 -n 1

CLIENTPIDS=()
for i in $(seq 1 1 $NUM_CLIENTS)
do
	../src/dw_client -L 1000 -n 5 &> /tmp/dw_client_loads_$i.log &
    CLIENTPIDS[${i}]=$!
done
...
echo "Full results in /tmp/dw_client_loads_*.log"
tail -n -10 /tmp/dw_client_loads_*.log
